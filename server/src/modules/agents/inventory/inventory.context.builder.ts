/**
 * Inventory Agent Context Builder — Phase 10: Inventory Intelligence Agent.
 */
...
export interface InventoryContextResult extends ContextBuilderResult {
...
export class InventoryContextBuilder {
  build(evidence: EvidenceResultItem[]): InventoryContextResult {
    ...
    const hasTemporalContext = evidence.some((e) => {
      ...
      const temporalPatterns = [/q[1-4]\s*\d{4}/, /\d{4}/, /quarter/, /annual/, /month/, /week/];
    ...
    logger.debug(
      ...
      '[InventoryContextBuilder] Inventory context assembled.',
    );
...
export const inventoryContextBuilder = new InventoryContextBuilder();
